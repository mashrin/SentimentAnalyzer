...
class NaiveBayesClassifier:
    ...
    def __init__(self, data, keyword, time, trainingDataFile, classifierDumpFile, trainingRequired = 0):
        ...
        self.helper = classifier_helper.ClassifierHelper('FeatureListFinal2.txt')
        ...
        if(trainingRequired):
            ...
            if(f1):
                self.classifier = np.load(f1,mmap_mode='rb')                
                f1.close()                
            else:
                self.classifier = self.getNBTrainedClassifer(trainingDataFile, classifierDumpFile)
    #end
    ...
    def getProcessedTweets(self, data):        
        ...
        for i in data:
            ...
            for t in d:
                #print t
                tw.append(self.helper.processTweet(t))
            tweets[i] = tw            
        #end loop
        return tweets
    #end
    ...
    def getFilteredTrainingData(self, trainingDataFile):
        ...
        count = 1       
        for row in reader:
            processed_tweet = self.helper.processTweet(row[1])
            sentiment = row[0]
    ...
    def classify(self):        
        for i in self.tweets:
            ...
            res = {}
            for t in tw:
                label = self.classifier.classify(self.helper.extract_features(self.helper.getFeatureVector(t)))
                if(label == 'positive'):
                    self.pos_count[i] += 1
                elif(label == 'negative'):                
                    self.neg_count[i] += 1
                    ...
                    self.neut_count[i] += 1
                result = {'text': t, 'tweet': self.tweets[i][count], 'label': label}
                res[count] = result
                count += 1
            #end inner loop
            self.results[i] = res
