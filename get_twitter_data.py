...
class TwitterData:
    #start __init__
    def __init__(self):
        self.currDate = datetime.datetime.now() #(Year("%Y"),Month("%m"),Day("%d"),Hours("%H"),Minutes("%M"),Seconds("%S"),Milliseconds("%f")) Eg:(2016, 4, 4, 22, 53, 53, 754000)
        self.weekDates = []
        self.weekDates.append(self.currDate.strftime("%Y-%m-%d")) #Year:Month:Day 2016-04-04
        for i in range(1,7):
    ...
    def getData(self, keyword, params = {}):
        ...
        #data={'screen_name':keyword,'count':maxTweets,'include_rts':'false','exclude_replies':'true'}
